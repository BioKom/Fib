/**
 * @file t_nD1_cPolynom_evaluePolynomHillClimbing
 * file name: t_nD1_cPolynom_evaluePolynomHillClimbing.cpp
 ...
 *
 *
 * This test is for the fib::algorithms::nLinearInequation::hillClimbingInRanges() .
 *
 */
/*
History:
...
int main(int argc, char* argv[]){
	...
	cout<<endl<<"Running Test for the function fib::algorithms::nLinearInequation::hillClimbingInRanges()"<<endl;
	cout<<      "========================================================================================"<<endl;
...
//	const long lMaxX = lMinX + (rand() % 1024) + 1;
	...
	const unsigned long ulMaxIteration = 256 * 256;
	for ( unsigned int uiMaxPolynomOrder = 1; uiMaxPolynomOrder <= 10;
			uiMaxPolynomOrder++ ){
		
		vector< cLinearConstrainFix<double> > vecOfInequationsOriginal( vecData.size(),
			cLinearConstrainFix<double>( uiMaxPolynomOrder, vecData.size() ) );
		vector< cRangeFactor<double> > vecYRanges;
		for( unsigned long iActualDataPoint = 0;
				iActualDataPoint < vecOfInequationsOriginal.size(); iActualDataPoint++ ){
			
			//set the values for the x/ a_i factors
			const double dX = vecData[ iActualDataPoint ].x;
			double dFactor = 1.0;
			...
			vecYRanges.push_back( cRangeFactor<double>(
				vecData[ iActualDataPoint ].minY, vecData[ iActualDataPoint ].maxY ) );
		}
		vector< cRangeFactor<double> > vecXRanges;
		
		vecXRanges.push_back( cRangeFactor<double>( -256.0, 256.0) );
		
		if ( 2 <= uiMaxPolynomOrder ){
			vecXRanges.push_back( cRangeFactor<double>( -8.0, 8.0) );
			
		}
		if ( 3 <= uiMaxPolynomOrder ){
			vecXRanges.push_back( cRangeFactor<double>( -8.0, 8.0) );
		}
		if ( 4 <= uiMaxPolynomOrder ){
			vecXRanges.push_back( cRangeFactor<double>( -8.0, 8.0) );
		}
		if ( 5 <= uiMaxPolynomOrder ){
			vecXRanges.push_back( cRangeFactor<double>( -4.0, 4.0) );
		}
		if ( 6 <= uiMaxPolynomOrder ){
			vecXRanges.push_back( cRangeFactor<double>( -4.0, 4.0) );
		}
		if ( 7 <= uiMaxPolynomOrder ){
			for ( unsigned int uiRangeFactor = 7;
					uiRangeFactor <= uiMaxPolynomOrder; uiRangeFactor++ ){
				vecXRanges.push_back( cRangeFactor<double>( -2.0, 2.0) );
			}
		}
		
		pair< vector<double>, double> paReturnedPolyFactors = hillClimbingInRanges(
			vecOfInequationsOriginal, vecXRanges, vecYRanges, ulMaxIteration );
			
		cPolynom<long, double> actualPolynom;
		actualPolynom.vecFactors = paReturnedPolyFactors.first;
