 ...
 */
bool cFunction::assignValues( const cFibElement & fibElement ){
	...
	const cFunction * pOtherFunction = ((const cFunction*)(&fibElement));
	const cUnderFunction * pOtherUnderFunction =
		pOtherFunction->pUnderFunction;
	if ( pOtherUnderFunction == NULL ){
		//set the underfunction of this function element to the null pointer NULL
		if ( pUnderFunction ){
			//delete old subfunction
			delete pUnderFunction;
			pUnderFunction = NULL;
		}
		return true;
	...
	const set< cFibVariable* > setUsedVariables = (const_cast< cUnderFunction * >(
		pOtherUnderFunction))->getUsedVariables();
	...
	//assign the values
	//copy subfunction of other function
	if ( pUnderFunction ){
		//delete old subfunction
		delete pUnderFunction;
	}
	pUnderFunction = pOtherUnderFunction->clone( NULL, this );
	...
	if ( ! liVariablesToReplace.empty() ){
		...
		for ( list< pair< cFibVariable * ,cFibVariable * > >::iterator
			...
			//replace variable in areavector
			if ( ! pUnderFunction->replaceVariable(
