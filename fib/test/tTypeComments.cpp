 ...
 * System: C++
 *
 * This test is for the class cTypeComments.
 *
 ...
 *
 *
 * This file contains the test for the class cTypeComments,
 * which represents the type for comments.
 *
 *
 *
 *  What's tested of class cTypeComments:
 * 	- cTypeComments()
...
int main(int argc,char* argv[]){
	...
	cout<<endl<<"Running Test for cTypeComments methods"<<endl;
	cout<<      "======================================"<<endl;
 ...
 * and getType() method of the cTypeComments class.
 *
...
int testCostructor( unsigned long &ulTestphase ){
	...
	ulTestphase++;
	cout<<endl<<"TESTPASE "<<ulTestphase<<" : Testing constructing simple cTypeComments"<<endl;

	cout<<"cTypeComments typeComments;"<<endl;
	cTypeComments typeComments;

	//check the getType() methode from cTypeComments
	if ( typeComments.getType() == (unsignedIntFib)(11) ){
	
		cout<<"The type of comments is correctly 11 . "<<endl;
	}else{
		cerr<<"Error: The type of comments is "<<
			typeComments.getType()<<" but should be 11 ."<<endl;
		iReturn++;
	}
...
int testIsCompatibleGetStandardDomain( unsigned long &ulTestphase ){
	...
	ulTestphase++;
	cout<<endl<<"TESTPASE "<<ulTestphase<<" : Testing the global standard domain for the cTypeComments"<<endl;
	...
	if ( typeComments.isCompatible( *domainInt ) ){
	...
	if ( ! typeComments.isCompatible( *vectorDomain ) ){
	...
	if ( ! typeComments.isCompatible( *vectorDomain1 ) ){
	...
	cDomain *standardDomain = typeComments.getStandardDomain();
...
int testClone( unsigned long &ulTestphase ){
	...
	cout<<"cTypeComments *typeCommentsClone1 = typeComments.clone();"<<endl;
	cTypeComments *typeCommentsClone1 = typeComments.clone();


	cout<<endl<<"checking the clone:"<<endl;
	//check the getType() methode from cTypeComments
	if ( typeCommentsClone1->getType() == (unsignedIntFib)(11) ){
	
		cout<<"The type of comments is correctly 11 . "<<endl;
	}else{
		cerr<<"Error: The type of comments is "<<
			typeCommentsClone1->getType()<<" but should be 11 ."<<endl;
		iReturn++;
	}
	delete typeCommentsClone1;
...
int testEqual( unsigned long &ulTestphase ){
	...
	//create cTypeCommentss to compare
	cout<<"cTypeComments typeComments1;"<<endl;
	cTypeComments typeComments1;
	cout<<"cTypeComments typeComments2;"<<endl;
	cTypeComments typeComments2;
	...
	//compare with typeComments1
	cTypeElement *actualType=&typeComments1;
	string szActualTypeName="typeComments1";
	iReturn += testCompareTwoEqualTyps( *actualType, szActualTypeName, typeComments1, "typeComments1" );
	iReturn += testCompareTwoEqualTyps( *actualType, szActualTypeName, typeComments2, "typeComments2" );
	...
	//compare with typeComments2
	actualType=&typeComments2;
	szActualTypeName="typeComments2";
	iReturn += testCompareTwoEqualTyps( *actualType, szActualTypeName, typeComments1, "typeComments1" );
	iReturn += testCompareTwoEqualTyps( *actualType, szActualTypeName, typeComments2, "typeComments2" );
	iReturn += testCompareTwoNotEqualTyps( *actualType, szActualTypeName, typeInVar1N1, "typeInVar1N1" );
	iReturn += testCompareTwoNotEqualTyps( *actualType, szActualTypeName, typeProperty, "typeProperty" );
	...
	iReturn += testCompareTwoNotEqualTyps( *actualType, szActualTypeName, typeComments1, "typeComments1" );
	iReturn += testCompareTwoNotEqualTyps( *actualType, szActualTypeName, typeComments2, "typeComments2" );
...
int testEqualElementType( unsigned long &ulTestphase ){
	...
	if ( typeComments1.equalElementType( typeComments1 ) ){
	
		cout<<"The typeComments1 is of the same type as typeComments1. "<<endl;
	}else{
		cerr<<"Error: The typeComments1 is not of the same type as typeComments1."<<endl;
		iReturn++;
	}
	if ( typeComments1.equalElementType( typeComments2 ) ){
	
		cout<<"The typeComments1 is of the same type as typeComments2. "<<endl;
	}else{
		cerr<<"Error: The typeComments1 is not of the same type as typeComments2."<<endl;
		iReturn++;
	}
	if ( ! typeComments1.equalElementType( typeInVar1 ) ){
	
		cout<<"The typeComments1 is not of the same type as typeInVar1. "<<endl;
	}else{
		cerr<<"Error: The typeComments1 is of the same type as typeInVar1."<<endl;
		iReturn++;
	}

	if ( ! typeComments1.equalElementType( typeProperty ) ){
	
		cout<<"The typeComments1 is not of the same type as typeProperty. "<<endl;
	}else{
		cerr<<"Error: The typeComments1 is of the same type as typeProperty."<<endl;
		iReturn++;
	}

	if ( ! typeComments1.equalElementType( typeDimension1D3 ) ){
	
		cout<<"The typeComments1 is not of the same type as typeDimension1D3. "<<endl;
	}else{
		cerr<<"Error: The typeComments1 is of the same type as typeDimension1D3."<<endl;
		iReturn++;
	}
...
int testXmlType( const string szFilename,
	...
	if ( pXmlElement ) {
		...
		if ( szElementName == "comments" ){
			cout<<"The root element is correctly named \"comments\". "<<endl;
		
		}else{
			cerr<<"Error: The name of the root element is "<< szElementName <<" and not \"comments\"."<<endl;
			iReturn++;
		}
...
int testStoreXml( unsigned long &ulTestphase ){
	...
	ulTestphase++;
	cout<<endl<<"TESTPASE "<<ulTestphase<<" : Testing storing a simple comments type"<<endl;
	...
	strcat( strcat( szFileNameBuffer,
		(char*)DIR_OUTPUT ), "typeComments1.xml" );
	ofstream * fileXml = new ofstream( szFileNameBuffer );
	
	cout<<"typeComments1.storeXml( *fileXml );"<<endl;
	bool bStoreSuccesfull = typeComments1.storeXml( *fileXml );
	delete fileXml;
	...
	ulTestphase++;
	cout<<endl<<"TESTPASE "<<ulTestphase<<" : Testing storing a 5'th comments type with a domain"<<endl;
	...
	szFileNameBuffer[0] = 0;
	strcat( strcat( szFileNameBuffer,
		(char*)DIR_OUTPUT ), "typeComments1Dom.xml" );
	fileXml = new ofstream( szFileNameBuffer );
	
	cout<<"typeComments1.storeXml( *fileXml, &domainNaturalNumberB4 );"<<endl;
	bStoreSuccesfull = typeComments1.storeXml( *fileXml, &domainNaturalNumberB4 );
	delete fileXml;
...
int testStore( unsigned long &ulTestphase ){
	...
	ulTestphase++;
	cout<<endl<<"TESTPASE "<<ulTestphase<<" : Testing storing a simple comments type for commentss"<<endl;
	...
	szFileNameBuffer[0] = 0;
	strcat( strcat( szFileNameBuffer,
		(char*)DIR_OUTPUT ), "typeComments1.fib" );
	ofstream * pFile = new ofstream( szFileNameBuffer, ios_base::out | ios_base::binary );
	
	char cRestBit = 0;
	unsigned char cNumberOfRestBit = 0 ;
	bool bStoreSuccesfull = typeComments1.store( *pFile, cRestBit, cNumberOfRestBit );
	(*pFile) << cRestBit;
	delete pFile;
	...
	//test stored data
	const char cTypeComments1[] = { (char)0x90,
		(char)0x00 }; //restbit
	iReturn += compareBytsWithFile( szFileNameBuffer, cTypeComments1, 2 );



	ulTestphase++;
	cout<<endl<<"TESTPASE "<<ulTestphase<<" : Testing storing a comments type with 5 restbitsbits"<<endl;
	...
	szFileNameBuffer[0] = 0;
	strcat( strcat( szFileNameBuffer,
		(char*)DIR_OUTPUT ), "typeComments1R5.fib" );
	pFile = new ofstream( szFileNameBuffer, ios_base::out | ios_base::binary );
	
	cRestBit = (char)0x05;
	cNumberOfRestBit = 5 ;
	bStoreSuccesfull = typeComments1.store( *pFile, cRestBit, cNumberOfRestBit );
	(*pFile) << cRestBit;
	delete pFile;
	...
	if ( ( ucRestBit == (unsigned char)((char)0x12) ) && ( cNumberOfRestBit == 5 ) ){
		cout<<"The restbit is \"(char)0x"<< hex << (unsigned short)cRestBit <<"\" the number of restbit is "<<
			dec << (unsigned short)cNumberOfRestBit <<". "<<endl;
	}else{
	...
	//test stored data
	const char cTypeComments5[] = { (char)0x05, (char)0x12 }; 
	iReturn += compareBytsWithFile( szFileNameBuffer, cTypeComments5, 2 );
