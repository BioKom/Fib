 ...
 */
cOpFindEvenLine::cOpFindEvenLine( const cOperationIdentifier & operationId ):
 ...
 */
cOpFindEvenLine::~cOpFindEvenLine(){
	//nothing to do
}
 ...
 */
cOpFindEvenLine * cOpFindEvenLine::createInstance(
		const cOperationIdentifier & operationId ) const{
	
	return new cOpFindEvenLine( operationId );
}
...
bool cOpFindEvenLine::run(){
	...
	if ( ( ! bObjectEvalued ) || (evaluedPositionsData.liEvaluedPositionData.size() < 3) ){
		delete pIndividual;
		setIsRunning( false );
		return true;
	}
 ...
 */
string cOpFindEvenLine::getName() const{
	return "cOpFindEvenLine";
}
