/**
 * @file cOpFindSimilarLine
 * file name: cOpFindSimilarLine.cpp
 ...
 */
cOpFindSimilarLine::cOpFindSimilarLine( const cOperationIdentifier & operationId ):
...
/**
 * Destructor of the class cOpFindSimilarLine.
 */
cOpFindSimilarLine::~cOpFindSimilarLine(){
	//nothing to do
}
 ...
 */
cOpFindSimilarLine * cOpFindSimilarLine::createInstance(
		const cOperationIdentifier & operationId ) const{
	
	return new cOpFindSimilarLine( operationId );
}
...
bool cOpFindSimilarLine::run(){
	...
	//choose direction
	const unsignedIntFib uiNumberOfDimensions = choosenPosition.getNumberOfElements();
	...
	/*remove all not needed points (all points not on the same line in the
	choosen direction)*/
	for ( itrPoint = evaluedPositionsData.liEvaluedPositionData.begin();
			itrPoint != evaluedPositionsData.liEvaluedPositionData.end(); ){
		
		if ( itrPoint->first.getNumberOfElements() == 0 ){
			//backgroundpoint is not in line
			itrPoint = evaluedPositionsData.liEvaluedPositionData.
				erase( itrPoint );
			continue;
		}
		for ( unsignedIntFib uiActualDimension = 1;
			...
			if ( uiNumberOfDimensions <= uiActualDimension ){
				//don't erase this point -> check the next point
				itrPoint++;
				//break;
			}
		}
	...
	DEBUG_OUT_L2(<<"domains evalued"<<endl<<flush);
 ...
 */
string cOpFindSimilarLine::getName() const{
	return "cOpFindSimilarLine";
}
