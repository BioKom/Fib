/**
 * @file cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast
 * file name: cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast.h
 ...
 *
 *
 * Evalues the areas with the same properties and combines a random one to
 * an complex fib -underobject.
...
#include "cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast.h"
 ...
 */
cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast::cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast( const cOperationIdentifier & operationId ):
...
/**
 * Destructor of the class cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast.
 */
cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast::~cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast(){
	//nothing to do
}
 ...
 */
cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast * cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast::createInstance(
		const cOperationIdentifier & operationId ) const{
	
	return new cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast( operationId );
}
 ...
 */
bool cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast::run(){
	...
	DEBUG_OUT_L2(<<"uiNumberOfDimensions: "<< pFibObject->getNumberOfDimensions() <<endl<<flush);
	if ( pFibObject->getNumberOfDimensions() != 2 ){
	...
	DEBUG_OUT_L2(<<"evalue the fib -object"<<endl<<flush);
	...
	const unsignedLongFib ulEvaluedPoints = evaluedPositionsData.liEvaluedPositionData.size();
	...
	for ( list< pair< cVectorPosition, list< cVectorProperty > > >::iterator
			...
			itrPoint != evaluedPositionsData.liEvaluedPositionData.end(); itrPoint++ ){
		
		//for every property
		for ( list< cVectorProperty >::iterator itrProperty = itrPoint->second.begin();
...
//#define DEBUG_OUTPUT_MATRIX
	...
	typedef pair< nD1::cDataPoint< longFib, longFib>, list< cVectorProperty > * >
				typePairPointProperties;
	typedef map< nD1::cDataPoint< longFib, longFib>, list< cVectorProperty > * >
	...
	const typeMapPointProperties::iterator itrOriginalPoint =
	...
	DEBUG_OUT_L2(<<"createSplineItrFastBorderAreasForPoints()"<<endl<<flush);
	cFibElement * pAreaFibObject = createSplineItrFastBorderAreasForPoints< longFib >(
		setCheckedPoints, pNewPoint, pVariableX, pVariableY, 4 );
	...
	if ( pAreaFibObject->getType() == 'l' ){
		//remove some of the smaler areas
		const unsignedIntFib uiNumberOfUnderobjects =
			((cList*)pAreaFibObject)->getNumberOfUnderobjects();
		
		const unsignedIntFib uiFirstUnderobjectsToRemove =
			uiNumberOfUnderobjects - (rand() % (uiNumberOfUnderobjects - 1));
		
		DEBUG_OUT_L2(<<"number of subareas "<<uiNumberOfUnderobjects<<
			" removing alle subareas from the "<<uiFirstUnderobjectsToRemove<<endl<<flush);
		
		if ( 1 < uiFirstUnderobjectsToRemove ){
			//more than two underobjects will remain
			((cList*)pAreaFibObject)->deleteUnderobjects(
				uiFirstUnderobjectsToRemove );
		}else{//just the biggest underobject will remain
			cList * pOldList = ((cList*)pAreaFibObject);
			
			pAreaFibObject = pOldList->getUnderobject( 1 );
			pOldList->deleteUnderobject( 1, false );
			
			pOldList->deleteObject();
		}
	}//else no subareas to remove
 ...
 */
string cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast::getName() const{
	return "cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast";
}
 ...
 * 	first pair is lower
 */
bool cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast::lowerPropertyElement(
 ...
 */
bool cOp2DFibFindEvenPartAreaSplineMax4BorderItrFast::isNull( const double & dValue ){
